...
export class TableComponent {
  ...
  addRecord() {
    const newEmployee = { id: this.employeez.length + 1, ...this.addUser.value, visibility: true };
    this.employeez.push(newEmployee);
    this.addUser.reset();
    // console.log("After adding:", this.employeez);
  ...
  updateEmployee(index: number) {
    // console.log("Printed");
    console.log(index);
    const employee = this.employeez[index];
  ...
  updateEmployeeData() {
    ...
    const index = this.employeez.findIndex(employee => employee.id === updatedEmployeeData.id);

    if (index !== -1) {
      this.employeez[index] = {
  ...
  toggleTag(index: number) {
    ...
    console.log("Delete button Bitted")
  }
}